...
//! [![Changelog.md](https://img.shields.io/badge/Keep%20a%20Changelog-Changelog.md-blue)](https://github.com/katharostech/conjecture/blob/master/CHANGELOG.md)
//!
//! # Conjecture
//!
//! Conjecture is a fork of the [Axiom](https://github.com/rsimmonsjr/axiom) actor framework that was made so we could use the awesome Actor framework while experimenting with our own ideas for Actor framework design.
//!
//! Conjecture brings a highly-scalable actor model to the Rust language based on the many lessons learned over years of Actor model implementations in Akka and Erlang. Conjecture is, however, not a direct re-implementation of either of the two aforementioned actor models but rather a new implementation deriving inspiration from the good parts of those projects.
//!
...
//!
//! Note that within the language of Rust, rule five cannot be enforced by Rust but is a best practice which is important for developers creating actors based on Conjecture. In Erlang and Elixir rule five cannot be violated because of the structure of the language but this also leads to performance limitations. It's better to allow internal mutable state and encourage the good practice of not sending mutable messages.
//!
...
//!
//! Although programming in the actor model is quite an involved process you can get started with Conjecture in only a few lines of code.
//!
//! ```rust
...
//! ```
//!
//! This code creates a named actor out of an arbitrary struct. Since the only requirement to make an actor is to have a function that is compliant with the [`conjecture::actors::Processor`] trait, anything can be an actor. If this struct had been declared somewhere outside of your control you could use it in an actor as state by declaring your own handler function and making the calls to the 3rd party structure.
//!
